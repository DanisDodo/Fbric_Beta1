...
@property (strong, nonatomic) UIButton *themeChooseButton;
...
@property (nonatomic) MemorialType tempType;
...
@synthesize themeChooseButton = _themeChooseButton;
@synthesize tempDate = _tempDate;
@synthesize tempImage = _tempImage;
@synthesize tempType = _tempType;
...
- (void)viewDidLoad
{
    ...
    self.themeLabel = [[UILabel alloc]initWithFrame:CGRectMake(CELL_X, TITLE_FIELD_Y+TITLE_FIELD_HEIGHT*2+CELL_INTERVAL*2, TITLE_FIELD_WIDTH-50, TITLE_FIELD_HEIGHT)];
    self.themeLabel.backgroundColor = [UIColor lightGrayColor];
    self.themeLabel.text = @"sdfsdf";
    self.themeChooseButton = [[UIButton alloc]initWithFrame:CGRectMake(320-50, TITLE_FIELD_Y+TITLE_FIELD_HEIGHT*2+CELL_INTERVAL*2, 40, 40)];
    self.themeChooseButton.backgroundColor = [UIColor redColor];
    [self.themeChooseButton addTarget:self action:@selector(chooseTheme) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.view addSubview:self.themeChooseButton];
}
...
-(void)editDone{
    ...
    [dataManager insertObjectWithTitle:title type:default_day startDate:self.tempDate cover:self.tempImage];
